...
assert grid.shape == (230, 150), grid.shape
...
# 9. Ego frame -----------------------------------------------------------------
print("\n9. ego frame")
mounted = GroundCalibration(intr, 2.4, pitch_deg=8.0, mount_x_m=2.9)
pm = BEVProjector(mounted, grid)

ec, er = grid.ego_cell
assert 0 <= ec < grid.cols and 0 <= er < grid.rows, (ec, er)
print(f"   ego (0,0) m -> cell ({ec:.0f}, {er:.0f}) inside {grid.shape}   OK")

uv = np.array([[400.0, 420.0]])
shift = pm.image_to_ground(uv)[0][0] - proj.image_to_ground(uv)[0][0]
assert abs(shift - 2.9) < 1e-6, shift
print(f"   mount offset shifts range by {shift:.2f} m exactly           OK")

for bad in [dict(x_min=0.0), dict(y_min=0.0), dict(x_min=2.0)]:
    try:
        BEVGrid(**bad)
    except ValueError:
        pass
    else:
        raise AssertionError(f"BEVGrid({bad}) should have been rejected")
print("   grids that exclude the ego are rejected                  OK")

obs = pm.observable_mask
ego = grid.ego_footprint_mask()
assert (obs & ego).sum() == 0, "ego body must never be reported as observable"
print(f"   ego body {ego.sum()} cells, never marked observable           OK")

rs = np.where(obs.any(axis=1))[0]
Xn, _ = grid.grid_to_ground(0, rs.max())
print(f"\n   BLIND ZONE: front bumper at 3.8 m, nearest visible ground {Xn:.1f} m")
print(f"   => {Xn - 3.8:.1f} m directly ahead of the bumper is invisible.")
print("      A child stepping off a kerb there is in NO frame of your BEV.")